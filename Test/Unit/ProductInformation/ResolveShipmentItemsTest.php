 ...
 * @internal
 */
class ResolveShipmentItemsTest extends ProductInformationTestCase
{
     ...
     * @return void
     */
    public function test_not_configurable_shipment_items_are_mapped_to_their_products()
    {
        ...
        $shipment = $this->createShipmentMock([
        ...
        $resolution = $this->sut->resolveProductVariants($shipment);

        $this->assertCount(5, $resolution);

        $shipmentItems = $shipment->getItems();
        $this->assertSame($shipmentItems[0], $resolution[0]['item'], 'Shipment item 0 should be the same as the first item in the resolution');
        $this->assertSame($shipmentItems[1], $resolution[1]['item'], 'Shipment item 1 should be the same as the first item in the resolution');
        $this->assertSame($shipmentItems[2], $resolution[2]['item'], 'Shipment item 2 should be the same as the first item in the resolution');
        $this->assertSame($shipmentItems[3], $resolution[3]['item'], 'Shipment item 3 should be the same as the first item in the resolution');
        $this->assertSame($shipmentItems[4], $resolution[4]['item'], 'Shipment item 4 should be the same as the first item in the resolution');
     ...
     * @return void
     */
    public function test_configurable_shipment_items_are_mapped_to_their_products()
    {
        $this->setUpProductRepository($this->productRepository, [
            ['getId' => 1, 'getSku' => 'sku1', 'getTypeId' => ProductTypeCodes::SIMPLE],
            ['getId' => 2, 'getSku' => 'sku2', 'getTypeId' => ProductTypeCodes::BUNDLE],
        ...
        $this->assertSame($shipmentItems[3], $resolution[3]['item'], 'Shipment item 3 should be the same as the first item in the resolution');
        $this->assertSame($shipmentItems[4], $resolution[4]['item'], 'Shipment item 4 should be the same as the first item in the resolution');
        $this->assertSame($shipmentItems[5], $resolution[5]['item'], 'Shipment item 5 should be the same as the first item in the resolution');
        $this->assertSame($shipmentItems[6], $resolution[6]['item'], 'Shipment item 6 should be the same as the first item in the resolution');
